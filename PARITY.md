...
The two plugins are mutually exclusive at runtime ([#55](https://github.com/Oveduumnakal/Pricewatch-Plugin/issues/55)) — Stockpile is a superset, so the client disables one when the other is enabled. That makes parity **more** important rather than less: a user picks one, and whichever they pick is the only one fixing their bug.
...
| Config group `pricewatch` | Sharing `stockpile` would corrupt both plugins' keys in the same RuneScape profile. Still required now the two conflict: each leaves its settings behind when disabled, and a user switching back and forth would find them overwritten. |
